...
import { Mic, MicOff, Send, Radio, ArrowLeft, Sparkles } from "lucide-react";
...
const ChatPage = () => {
  ...
  const { status: voiceStatus, interimTranscript, error: voiceError, isSupported, startListening, stopListening } =
    useVoiceInput({ lang, onTranscript: handleVoiceTranscript });
  ...
  return (
    <div className="relative flex flex-col h-screen bg-background max-w-md mx-auto overflow-hidden">
      ...
      <header className="relative z-10 flex items-center justify-between px-4 py-3 border-b border-border/50 glass-card-strong">
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-full gold-gradient text-xs font-semibold text-primary-foreground transition-all active:scale-95 shadow-md shadow-primary/20"
          >
      ...
      <div className="relative z-10 flex-1 overflow-y-auto px-4 py-4 space-y-3">
        ...
        {allMessages.map((msg) => (
          ...
          >
            <div className="flex flex-col gap-0.5">
              ...
              <span
                className={`text-[9px] text-muted-foreground px-1 ${msg.isUser ? "text-right" : "text-left"}`}
              >
                {msg.time}
              </span>
            </div>
        ...
        {/* Промежуточная транскрипция при голосовом вводе */}
        {isListening && interimTranscript && (
          <div className="flex justify-end animate-fade-up">
            <div className="max-w-[80%] px-4 py-2.5 text-sm leading-relaxed text-muted-foreground italic rounded-2xl rounded-br-md border border-primary/20 bg-primary/5">
              {interimTranscript}…
            </div>
          </div>
        )}
      ...
      {/* Voice error */}
      {voiceError && (
        <div className="relative z-10 mx-4 mb-1 px-3 py-2 rounded-xl bg-destructive/10 border border-destructive/20 text-xs text-destructive">
          {voiceError}
        </div>
      )}
      ...
      <div className="relative z-10 px-4 pb-2 pt-2 border-t border-border/50 glass-card-strong">
        {isListening && (
          <div className="flex items-center gap-1.5 mb-2 px-1">
            ...
            <span className="text-xs text-muted-foreground">Слушаю...</span>
          </div>
        )}
        <div className="flex items-center gap-2">
          <div className="flex-1 flex items-center bg-secondary/30 border border-border/50 rounded-2xl px-4 py-2.5 focus-within:border-primary/30 transition-colors">
            <input
              ...
              placeholder={isListening ? "Говорите..." : t.chat.placeholder}
          ...
          {input.trim() ? (
            <button
              onClick={handleSend}
              className="w-11 h-11 rounded-full gold-gradient flex items-center justify-center text-primary-foreground transition-all active:scale-90 shadow-md shadow-primary/20"
            >
            ...
            <button
              ...
              className={`w-11 h-11 rounded-full flex items-center justify-center transition-all active:scale-90 ${
                isListening
                  ? "bg-[hsl(var(--mic-active))] mic-pulse shadow-md"
                  : "bg-secondary/30 border border-border/50 text-muted-foreground hover:text-foreground hover:border-primary/30"
              }`}
            >
              {isListening ? (
            ...
            <button
              disabled
              className="w-11 h-11 rounded-full flex items-center justify-center bg-secondary/20 border border-border/30 text-muted-foreground/40 cursor-not-allowed"
            >
              <Mic className="w-5 h-5" />
            </button>
          )}
