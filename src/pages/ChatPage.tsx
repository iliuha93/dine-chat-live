...
const ChatPage = () => {
  ...
  const addMessage = useCallback((text: string, isUser: boolean) => {
    setMessages((prev) => [
      ...prev,
      { id: Date.now().toString(), text, isUser, time: getTime() },
    ]);
    scrollToBottom();
  }, []);
  ...
  const handleSend = () => {
    ...
    setInput("");
    simulateReply("Спасибо за ваш вопрос! Наш ассистент скоро ответит. 😊");
  };
  ...
  const stopRecording = () => {
    ...
    if (isRecording) {
      ...
      simulateReply("Я получил ваше голосовое сообщение. Обрабатываю...");
    }
  };
  ...
  return (
    <div className="relative flex flex-col h-screen bg-background max-w-md mx-auto overflow-hidden">
      ...
      <header className="relative z-10 flex items-center justify-between px-4 py-3 border-b border-border/50 glass-card-strong">
        ...
        <div className="flex items-center gap-2.5">
          <div className="relative">
            ...
            <div className="absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-background" style={{ backgroundColor: "hsl(142, 71%, 45%)" }} />
          </div>
          <div>
            <h2 className="text-sm font-semibold text-foreground leading-tight">La Maison</h2>
            <div className="flex items-center gap-1">
              <Sparkles className="w-2.5 h-2.5 text-primary" />
              <p className="text-[10px] text-primary font-medium">ИИ-ассистент</p>
            </div>
          </div>
        </div>
        ...
        {messages.map((msg) => (
          ...
          >
            <div className="flex flex-col gap-0.5">
              ...
              <span className={`text-[9px] text-muted-foreground px-1 ${msg.isUser ? "text-right" : "text-left"}`}>
                {msg.time}
              </span>
            </div>
          </div>
      ...
      {/* Input */}
      <div className="relative z-10 px-4 pb-6 pt-2 border-t border-border/50 glass-card-strong">
