...
const WaveVisualizer = () => (
  <div className="flex items-center justify-center gap-1 h-8">
    {Array.from({ length: 12 }).map((_, i) => (
      <div key={i} className="w-1 bg-primary/80 rounded-full wave-bar" style={{ animationDelay: `${i * 0.1}s`, height: "8px" }} />
    ))}
  </div>
);

const BotAvatar = ({ isSpeaking, t }: { isSpeaking: boolean; t: any }) => (
  <div className="relative flex flex-col items-center gap-4">
    ...
    <div className={`relative w-28 h-28 rounded-full glass-card-strong flex items-center justify-center transition-all duration-500 ${isSpeaking ? "shadow-2xl shadow-primary/30" : ""}`}>
      ...
      <div className="absolute bottom-7 flex items-center justify-center">
        <div className={`rounded-full bg-primary/80 transition-all duration-150 ${isSpeaking ? "w-6 bot-mouth-speaking" : "w-4 h-1.5"}`} />
      </div>
    </div>
    <p className="text-xs text-muted-foreground font-medium tracking-wider uppercase">
      {isSpeaking ? t.live.speaking : t.live.ai_assistant}
    </p>
  </div>
);
    ...
    <div className="relative flex flex-col h-screen bg-background max-w-md mx-auto overflow-hidden">
      ...
      <div className="relative z-10 flex-1 flex flex-col items-center justify-center gap-8 px-6">
        <div className="text-center space-y-2">
          <h1 className="text-3xl font-bold gold-text" style={{ fontFamily: "var(--font-display)" }}>{t.live.heading}</h1>
          <p className="text-sm text-muted-foreground max-w-[260px]" style={{ fontFamily: "var(--font-elegant)" }}>{t.live.description}</p>
        </div>

        <BotAvatar isSpeaking={isActive} t={t} />
        ...
        <div className="relative">
          ...
          >
            {isActive ? <MicOff className="w-8 h-8 text-foreground" /> : <Mic className="w-8 h-8 text-primary-foreground" />}
          </button>
        </div>

        <p className="text-xs text-muted-foreground">{isActive ? t.live.listening : t.live.tap_to_start}</p>

        {isActive && (
          <div className="w-full space-y-3 animate-fade-up">
            <WaveVisualizer />
            <div className="glass-card-strong rounded-2xl p-4">
              <p className="text-[10px] text-primary font-medium uppercase tracking-wider mb-1.5">{t.live.transcription}</p>
              <p className="text-sm text-foreground/60 italic" style={{ fontFamily: "var(--font-elegant)" }}>{t.live.waiting}</p>
            </div>
          </div>
        )}
      </div>
